...
class ScenarioMachine:
    ...
    async def migrate_to_scene(self, scene: BaseScene, event, *, chat_id: int, user_id: int) -> None:
    ...
    async def refresh_current_scene(self, event, *, chat_id: int, user_id: int) -> None:

        scene = await self.get_current_scene(chat_id=chat_id, user_id=user_id)
        await scene.process_enter(event)

    async def execute_next_transition(self, event, *, chat_id: int, user_id: int,
                                      handler: Callable, direction: Optional[str] = None) -> None:
        ...
        try:
            async with self._lock_storage.acquire(chat_id=chat_id, user_id=user_id):
                ...
                await self._process_transition(event, magazine, chat_id=chat_id, user_id=user_id,
    ...
    async def execute_back_transition(self, event, *, chat_id: int, user_id: int) -> None:
    ...
    async def _process_transition(self, event, magazine: StateMagazine, *, chat_id: int, user_id: int,
